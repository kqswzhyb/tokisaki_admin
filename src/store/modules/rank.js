...
const actions = {
  getRank({ commit, rootState }, res) {
    const rank = {
      week: res.data.weekList
        ? `${
            res.data.weekList
              ? res.data.weekList.findIndex(
                  item => item.id === rootState.user.info.user.id
                ) === -1
                ? 0
                : res.data.weekList.findIndex(
                    item => item.id === rootState.user.info.user.id
              ...
              : -1 + 1
          } / ${res.data.weekList ? res.data.weekList.length : 0}`
        : "0 / 0",
      month: res.data.monthList
        ? `${
            res.data.monthList
              ? res.data.monthList.findIndex(
                  item => item.id === rootState.user.info.user.id
                ) === -1
                ? 0
                : res.data.monthList.findIndex(
              ...
              : -1 + 1
          } / ${res.data.monthList ? res.data.monthList.length : 0}`
        : "0 / 0",
      total: `${
        res.data.allList
          ? res.data.allList.findIndex(
              item => item.id === rootState.user.info.user.id
            ) === -1
            ? 0
            : res.data.allList.findIndex(
          ...
          : -1 + 1
      } / ${res.data.allList ? res.data.allList.length : 0}`
    };
